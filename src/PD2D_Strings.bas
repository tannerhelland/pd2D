Attribute VB_Name = "PD2D_Strings"
'***************************************************************************
...
Public Function LeftMatches(ByRef srcString As String, ByRef desiredString As String, Optional ByVal ignoreCase As Boolean = False) As Boolean
    If (LenB(srcString) >= LenB(desiredString)) Then LeftMatches = PD2D_Strings.StringsEqual(Left$(srcString, Len(desiredString)), desiredString, ignoreCase)
End Function
...
Public Function RightMatches(ByRef srcString As String, ByRef desiredString As String, Optional ByVal ignoreCase As Boolean = False) As Boolean
    If (LenB(srcString) >= LenB(desiredString)) Then RightMatches = PD2D_Strings.StringsEqual(Right$(srcString, Len(desiredString)), desiredString, ignoreCase)
End Function
...
Public Function StringFromMysteryBytes(ByRef srcBytes() As Byte, ByRef dstString As String, Optional ByVal forceWindowsLineEndings As Boolean = True) As Boolean
    ...
    If (srcBytes(0) = &HEF) And (srcBytes(1) = &HBB) And (srcBytes(2) = &HBF) Then
        ...
        dstString = Mid$(PD2D_Strings.StringFromUTF8(srcBytes), 2)
    ...
    Else
        ...
        If PD2D_Strings.AreBytesUTF8(srcBytes) Then
            dstString = PD2D_Strings.StringFromUTF8(srcBytes)
        ...
        Else
            ...
            Debug.Print "FYI: PD2D_Strings.StringFromMysteryBytes received a string with unclear encoding.  Current user's codepage was assumed."
        End If
...
StringConversionFailed:
    ...
    InternalError "PD2D_Strings.StringFromMysteryBytes() failed; string conversion abandoned.", Err.Number
...
Public Function StringFromUTF8(ByRef Utf8() As Byte, Optional ByVal customDataLength As Long = -1) As String
    ...
    If (lenWideString = 0) Then
        InternalError "PD2D_Strings.StringFromUTF8() failed because MultiByteToWideChar did not return a valid buffer length (#" & Err.LastDllError & ")."
    ...
    Else
        ...
        If (lenWideString = 0) Then
            InternalError "PD2D_Strings.StringFromUTF8() failed because MultiByteToWideChar could not perform the conversion, despite returning a valid buffer length (#" & Err.LastDllError & ")."
            StringFromUTF8 = vbNullString
        End If
...
Public Function StringFromUTF8Ptr(ByVal srcUtf8Ptr As Long, ByVal srcUTF8Len As Long) As String
    ...
    If (lenWideString = 0) Then
        InternalError "PD2D_Strings.StringFromUTF8Ptr() failed because MultiByteToWideChar did not return a valid buffer length (#" & Err.LastDllError & "); passed buffer length was " & srcUTF8Len & "."
    ...
    Else
        ...
        If (MultiByteToWideChar(CP_UTF8, 0, srcUtf8Ptr, srcUTF8Len, StrPtr(StringFromUTF8Ptr), lenWideString) = 0) Then
            InternalError "PD2D_Strings.StringFromUTF8Ptr() failed because MultiByteToWideChar could not perform the conversion, despite returning a valid buffer length (#" & Err.LastDllError & ")."
            StringFromUTF8Ptr = vbNullString
        End If
    End If
...
Public Function StringFromUTF16_FixedLen(ByVal srcPointer As Long, ByVal lengthInBytes As Long, Optional ByVal trimNullChars As Boolean = True) As String
    ...
    If trimNullChars Then StringFromUTF16_FixedLen = PD2D_Strings.TrimNull(StringFromUTF16_FixedLen)
End Function
...
Public Function StringsEqual(ByRef firstString As String, ByRef secondString As String, Optional ByVal ignoreCase As Boolean = False) As Boolean
    ...
    Else
        ...
        Else
            ...
            Else
                StringsEqual = PD2D.MemCmp(StrPtr(firstString), StrPtr(secondString), LenB(firstString))
            End If
        End If
    End If
...
'Compare a target string against an arbitrary list of PD2D_Strings.  Returns TRUE if the target string
' matches *at least one* of the arbitrary list of PD2D_Strings.
...
Public Function StrStrBM(ByRef strHaystack As String, ByRef strNeedle As String, Optional ByVal startSearchPos As Long = 1, Optional ByVal useFullUnicodeSet As Boolean = False, Optional ByVal terminalLengthHaystack As Long = -1) As Long
    ...
    'Wrap VB arrays around the needle and haystack PD2D_Strings.  This allows for faster traversal.
...
Public Function UTF8FromString(ByRef srcString As String, ByRef dstUtf8() As Byte, ByRef lenUTF8 As Long, Optional ByVal baseArrIndexToWrite As Long = 0, Optional ByVal trimTrailingNulls As Boolean = False) As Boolean
    UTF8FromString = PD2D_Strings.UTF8FromStrPtr(StrPtr(srcString), -1, dstUtf8, lenUTF8, baseArrIndexToWrite)
...
Public Function UTF8FromStrPtr(ByVal srcPtr As Long, ByVal srcLenInChars As Long, ByRef dstUtf8() As Byte, Optional ByRef lenUTF8 As Long, Optional ByVal baseArrIndexToWrite As Long = 0) As Boolean
    ...
    If (lenUTF8 <= 0) Then
        InternalError "PD2D_Strings.UTF8FromStrPtr() failed because WideCharToMultiByte did not return a valid buffer length (#" & Err.LastDllError & ", " & srcLenInChars & ")"
    ...
    Else
        ...
        If (Not UTF8FromStrPtr) Then InternalError "PD2D_Strings.UTF8FromStrPtr() failed because WideCharToMultiByte could not perform the conversion, despite returning a valid buffer length (#" & Err.LastDllError & ")."
